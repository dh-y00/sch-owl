...
import com.sch.owl.permission.annotation.DataScopeNew;
 ...
 */
@Aspect
public class DataScopeNewAspect
{
    /**
     * 全部数据权限
     */
    ...
    public void doBefore(JoinPoint point, DataScopeNew controllerDataScope) throws Throwable
    {
        clearDataScope(point);
        handleDataScope(point, controllerDataScope);
    }

    protected void handleDataScope(final JoinPoint joinPoint, DataScopeNew controllerDataScope)
    {
        // 获取当前的用户
        LoginUser loginUser = SecurityUtils.getLoginUser();
        if (StringUtils.isNotNull(loginUser))
        {
            LoginUserInfo currentUser = loginUser.getUser();
            // 如果是超级管理员，则不过滤数据
    ...
    public void dataScopeFilter(JoinPoint joinPoint, LoginUserInfo user, String deptAlias, String userAlias, String permission)
    {
        ...
        for (RoleInfoVo role : user.getRoles())
        {
            ...
            {
                List<Long> deptIdFromDept = remoteUpmsServe.getDeptIdAndChildDeptId(user.getDeptId());
